...
pub fn _fill_send_buffers<R: Read>(
...
pub fn _decode_send_buffer(
    ...
    match empty_receiver.recv() {
        Ok((_, mut buffer)) => {
            ...
            let in_read = decompressor.total_in();
            let out_written = decompressor.total_out();
            debug!(
                "in_read: {} - out_written: {} - total_bytes: {}",
                in_read,
                out_written,
                total_bytes
            );
            let status = decompressor
                .decompress(&compressed_buffer, &mut buffer[total_bytes..], FlushDecompress::None)
                .unwrap();

            total_bytes += (decompressor.total_out() - out_written) as usize;
            let mut lines = memchr_iter(b'\n', &buffer[..total_bytes]).collect::<Vec<usize>>();
            //let mut lines = memchr_iter(b'\n', &buffer[..total_bytes]);
            //debug!("total read bytes: {}  -  lines: {}  - needed lines: {}", total_bytes, lines.count(), read_cnt * 4);
            //let mut lines = memchr_iter(b'\n', &buffer[..total_bytes]);
            if lines.len() >= read_cnt * 4 || force {
                ...
                return (
                    (decompressor.total_in() - in_read) as usize,
                    true,
...
pub fn _send_buffer(
    ...
    match empty_receiver.recv() {
        Ok((_, mut buffer)) => {
            ...
            let lines = memchr_iter(b'\n', &buffer[..src_buffer.len()]).collect::<Vec<usize>>();
            match full_sender.send((src_buffer.len(), buffer, lines)) {
                Ok(_) => {
                    //debug!("Sending: {} bytes", last_sent_endline);
                    //debug!("kept: {}",extra_len);
                }
